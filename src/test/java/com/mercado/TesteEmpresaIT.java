...
public class TesteEmpresaIT {
	...
	public void SetUp() {
		...
		jsonEmpresaIncorretaSemNome = ResourceUtils.getContentFromResource("/json/incorreto/empresa/empresa-incorreta-sem-nome.json");
		jsonEmpresaIncorretaSemRazaoSocial = ResourceUtils.getContentFromResource("/json/incorreto/empresa/empresa-incorreta-sem-razaoSocial.json");
	}
	...
	public void deveRetornarStatus200_QuandoConsultarEmpresaExistente() {
		given()
			.accept(ContentType.JSON)
			.pathParam("id", empresaAutoPecas.getId())
		.when()
			.get("/{id}")
		.then()
			.statusCode(HttpStatus.OK.value());
	...
	public void deveRetornarStatus404_QuandoConsultarEmpresaInexistente() {
		given()
			.accept(ContentType.JSON)
			.pathParam("id", idEmpresaInexistente)
		.when()
			.get("/{id}")
		.then()
	...
	public void deveRetornarStatus400_QuandoCadastrarEmpresaSemNome() {
		given()
			.body(jsonEmpresaIncorretaSemNome)
			.accept(ContentType.JSON)
			.contentType(ContentType.JSON)
		.when()
			.post()
	...
	public void deveRetornarStatus400_QuandoCadastrarEmpresaSemRazaoSocial() {
		given()
			.body(jsonEmpresaIncorretaSemRazaoSocial)
			.accept(ContentType.JSON)
			.contentType(ContentType.JSON)
		.when()
			.post()
